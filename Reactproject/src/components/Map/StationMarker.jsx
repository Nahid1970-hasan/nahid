...
import { deletaStationData, loadNDSelSatationData } from "../../features/nationalDashboard/national_dashboard";
...
`
export default function StationMarker({ data }) {
    ...
    function onMarkerClick(event) {
        ...
        dispatch(loadNDSelSatationData(dData));
        dispatch(deletaStationData());
        setInfoOpen(!infoOpen);

    }

    return (<React.Fragment>
        <MarkerF onClick={onMarkerClick} icon={data.org_name ? SCG(data?.org_name ? data?.org_name?.toLowerCase() : "bmd", data?.cum_rain ?? "0") : ""} options={{}} animation={google.maps.Animation.DROP} title={data.station_name_en} position={data}>
            
                {infoOpen && <InfoWindowF position={data}>
                    <div ref={wraperRef} style={{ padding: "0 10px 10px", minWidth: "220px", color: "#000000" }}>
                        ...
                        {ndData.divloading == "pending" ? <Loader color="loader" /> :
                            <div>
                                <LBSpan margin="0 0 5px 0" lineHight="1.8" uLine="1.8px solid green">{ndData?.stationData?.datetime_en ? "At " + ndData?.stationData?.datetime_en ?? <Hypend /> + "BST" : ""} </LBSpan>
                                <LBSpan>{"Temperature: "} { getTempValue(ndData?.stationData?.temperature_en??"0") ? ndData?.stationData?.temperature_en + "°C" : <Hypend />}</LBSpan>
                                <LBSpan>{"Hourly Rainfall: "} {ndData?.stationData?.rain_gauge_en ? ndData?.stationData?.rain_gauge_en + " mm" : <Hypend />} </LBSpan>
                                <LBSpan>{"Daily Rainfall: "} {ndData?.stationData?.cum_rain ? ndData?.stationData?.cum_rain + " mm" : <Hypend />} </LBSpan>
                                <LBSpan>{"Wind Speed: "} {getWindValue(ndData?.stationData?.wind_speed_en??"0") ? ndData?.stationData?.wind_speed_en + " kph" : <Hypend />}</LBSpan>
                                <LBSpan>{"Wind Direction: "} {ndData?.stationData?.wind_direction_en ? ndData?.stationData?.wind_direction_en + "°" : <Hypend />}</LBSpan>
                                <LBSpan>{"Solar Radiation: "} {ndData?.stationData?.radiation_en ? ndData?.stationData?.radiation_en + " W/m2" : <Hypend />} </LBSpan>
                                <LBSpan>{"Relative Humidity: "} {ndData?.stationData?.humidity_en ? ndData?.stationData?.humidity_en + "%" : <Hypend />} </LBSpan>
                                <LBSpan>{"Surface Pressure: "} {ndData?.stationData?.pressure_en ? ndData?.stationData?.pressure_en + " hPa" : <Hypend />} </LBSpan>
                            </div>}
                    </div>
                </InfoWindowF>}
